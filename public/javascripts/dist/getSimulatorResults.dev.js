...
function getSimulatorResults(rows) {
  ...
  var completeResults = {
    emissions: {
      title: rows[79][0],
      intro: rows[80][1],
      graphs: [setCompoObject(rows, 81, 89), setCompoObject(rows, 94, 97)]
    },
    energieFinale: {
      title: rows[102][0],
      intro: rows[103][1],
      graphs: [setCompoObject(rows, 105, 112)]
    },
    energieRenouvelable: {
      title: rows[117][0],
      intro: rows[118][1],
    ...
    energieFacture: {
      title: rows[134][0],
      intro: rows[135][1],
      graphs: [setCompoObject(rows, 137, 154)]
    },
    polluants: {
      title: rows[160][0],
      intro: rows[161][1],
      graphs: [setCompoObject(rows, 163, 170), setCompoObject(rows, 175, 182), setCompoObject(rows, 187, 194), setCompoObject(rows, 199, 206), setCompoObject(rows, 211, 218)]
    }
  };
  return {
